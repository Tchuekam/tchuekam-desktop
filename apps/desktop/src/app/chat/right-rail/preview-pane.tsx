    ...
    <PreviewEmptyState
      ...
      secondaryAction={
        onRestartServer
          ? {
              disabled: restarting,
              label: restarting ? 'TchuekaM is restarting...' : 'Ask TchuekaM to restart the server',
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    try {
      ...
      appendConsoleEntry({
        level: 1,
        message: `TchuekaM is looking for a preview server to restart (${taskId})`
      })

      notify({
        kind: 'info',
        title: 'Restarting preview server',
        message: 'TchuekaM is working in the background. Watch the preview console for progress.',
        durationMs: 4000
      })
    } catch (error) {
      appendConsoleEntry({
  ...
  useEffect(() => {
      ...
      message:
        previewServerRestart.status === 'running'
          ...
          : previewServerRestart.status === 'complete'
            ? `TchuekaM finished restarting the preview server${
    ...
    if (previewServerRestart.status === 'complete') {
      ...
      notify({
        ...
        message: previewServerRestart.message?.slice(0, 200) || 'TchuekaM could not restart the server.',
        durationMs: 6000
      })
    }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      failPreviewServerRestart(
        taskId,
        'TchuekaM is still working, but no restart result has arrived yet. The server command may be running in the foreground.'
      )
